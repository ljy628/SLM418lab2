...
#Teams2019 <- Teams %>% filter(teamID == "CIN")
Teams2019
...
Teams20_19 <- Teams %>% filter(lgID == "NL" & divID == "C" & yearID >=2000& yearID <=2019)
Teams20_19 <- inner_join(Teams20_19, MLB_Team_Logo, by = "teamID")
ggplot(Teams20_19) +
  geom_line(aes(x = yearID, y = R, fill=teamID, group=teamID,colour = teamID)) +
...
plot1 <- ggplot(Teams20_19, aes(R, W)) +
  geom_line(aes(fill=teamID, group=teamID,colour = teamID)) +   # alpha modifies the transparency; size modifies the size of marks; shape=1 is the circle; stroke modifies the thickness
  labs(title = "Team Performance in NLC 2000-2019", x = "Seasons", y = "Wins") +
  transition_time(yearID) +
  ease_aes()+
  scale_y_continuous(position = "left")+ 
